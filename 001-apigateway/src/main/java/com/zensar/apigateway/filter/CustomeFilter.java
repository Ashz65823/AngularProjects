...
/**
 * 
 * @author AA65823 GlobalFilter is an interface given by API gateway
 * 
 */
//@Configuration //-->@Configuration is commented and this will help us in not sending header in postman and we can get output on browser.. If its not commented then we can only access apigateway in postman by sending header authorization 	
...
//This program get executed only when @Configuration is uncommented
		...
		if (list != null) {//If header not present this application will give 401 status which is UNAUTHORIZED
